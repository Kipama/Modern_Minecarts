...
public class ModernMinecartsConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        this.configList = this.addRenderableWidget(new ConfigList(this.width, footerTop - listTop, listTop, ROW_HEIGHT));
    ...
    private void saveAndClose() {
        try {
            ...
            ModernMinecartsConfig.setEnableRailCrossing(this.enableRailCrossing.getValue());
            ModernMinecartsConfig.setEnablePoweredDetectorRail(this.enablePoweredDetectorRail.getValue());
            ModernMinecartsConfig.setEnableRailJump(this.enableRailJump.getValue());
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        int footerTop = this.height - FOOTER_HEIGHT;
        graphics.fill(0, footerTop, this.width, this.height, 0xAA000000);
        super.extractRenderState(graphics, mouseX, mouseY, a);
        graphics.text(this.font, this.title, this.width / 2 - this.font.width(this.title) / 2, (TITLE_HEIGHT - this.font.lineHeight) / 2, 0xFFFFFF);
        if (this.statusMessage != null) {
            graphics.text(this.font, this.statusMessage, this.width / 2 - this.font.width(this.statusMessage) / 2, footerTop + 8, 0xFF5555);
        }
    }
    ...
    private class ConfigList extends ContainerObjectSelectionList<ConfigEntry> {
        ConfigList(int width, int height, int y, int itemHeight) {
            super(ModernMinecartsConfigScreen.this.minecraft, width, height, y, itemHeight);
        }
